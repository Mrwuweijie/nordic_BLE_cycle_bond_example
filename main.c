...
pm_peer_id_t m_wait_for_peer_removal = PM_PEER_ID_INVALID;
...
static void pm_evt_handler(pm_evt_t const * p_evt)
{
    ...
    switch (p_evt->evt_id)
    {
        case PM_EVT_BONDED_PEER_CONNECTED:
        {
            ...
            NRF_LOG_INFO("rank peer in PM_EVT_BONDED_PEER_CONNECTED: %d \r\n",p_evt->peer_id);
        ...
        case PM_EVT_CONN_SEC_SUCCEEDED:
        {
            ...
            {
                NRF_LOG_INFO("rank peer in PM_EVT_CONN_SEC_SUCCEEDED: %d \r\n",p_evt->peer_id);
                // Bump peer rank.
                APP_ERROR_CHECK(pm_peer_rank_highest(p_evt->peer_id));
		    }

            if (conn_sec_status.encrypted || conn_sec_status.bonded)
            {
            ...
            {
                // The peer did not encrypted or bonded, disconnect.
				NRF_LOG_INFO("Collector did not use encrypted or bonded, disconnecting");
		...
		{    
			if ((m_wait_for_peer_removal != PM_PEER_ID_INVALID) &&
                (m_wait_for_peer_removal == p_evt->peer_id))
            {
                NRF_LOG_INFO("m_coms_ble_adv: Peer %d Deleted: Advertising...", p_evt->peer_id);
                m_wait_for_peer_removal = PM_PEER_ID_INVALID;
                advertising_start();
            }
		}	break;
...
static void advertising_start(void)
{
	 //ensure this advertising_start() is called when starting advertising each time
	uint32_t err_code;
	uint32_t peer_count = pm_peer_count();
	
   //before advertising delete the least used peer to make room the for new bonding
   if (peer_count >= CONFIG_MAX_BONDS)
   {
        pm_peer_id_t peer_id;

        err_code = pm_peer_ranks_get(NULL, NULL, &peer_id, NULL);
        APP_ERROR_CHECK(err_code);
        ...
        m_wait_for_peer_removal = peer_id;
